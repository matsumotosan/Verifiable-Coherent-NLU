...
def get_tensor_dataset_tiered(dataset, max_sentences, add_segment_ids=False):
  ...
  all_input_mask = torch.tensor(np.array([[[[story['entities'][e]['input_mask'][s] if e < len(story['entities']) else np.zeros((seq_length)) for s in range(max_sentences)] for e in range(max_entities)] for story in ex_2s['stories']] for ex_2s in dataset]))
